...
var autoLayout_album = {
    ...
    init: function(options, sta, dataObj) {
        ...
        if (typeof options == "object") {//第一次加载，divId = 0 的时候
            ...
            if (document.getElementById(opts.leftID)) {
               ...
               // heightArray[0] = (leftBoard.offsetHeight + autoLayout_album.marginTop + parseInt(leftBoard.style.marginTop))
        ...
        function initContent(json) {
            ...
            len = picBoard.length; j < len; j++) {
				...
				var _imgCount = picBoard[j].img_count > 0 ? '<span class="number floatright">('+picBoard[j].img_count+')</span>' : '';
				
				if(picBoard[j].isMe == 1)
				{
					_isMeFollow = '<div class="follow"><a href="javascript:;" onclick="$.Edit_Album('+ picBoard[j].id +')" class="btn btn13 wbtn unfollow"><strong>编辑</strong><span</span></a></div>';
				}
				else
				{
					if(picBoard[j].is_follow_album)
					{
						_isMeFollow = '<div class="follow"><a href="javascript:;" onclick="$.Follow_Album('+ picBoard[j].id +',this)" class="btn btn13 wbtn unfollow"><strong>取消关注</strong><span></span></a></div>';
					}
					else
					{
						_isMeFollow = '<div class="follow"><a href="javascript:;" onclick="$.Follow_Album('+ picBoard[j].id +',this)" class="btn btn13 wbtn unfollow"><strong>关注</strong><span></span></a></div>';
					}
				}
                ...
                var _to = rows + "px";
                var inserDiv = ['<div id="' + autoLayout_album.divId + '" columns="' + cols + '" class="mbox" style="left:' + _le + ";top:" + _to + '; "><div class="followlist">', '<h3 class="mboxTitle">'+_imgCount+'<span class="t" title="d">'+picBoard[j].title+'</span></h3>', ''+picBoard[j].imgs+_isMeFollow+'</div></div>'].join("");
    ...
    updateDiv: function() {
        ...
        if (document.getElementById(k.leftID)) {
            ...
            n.style.left = "0px";
            n.style.top = f[0] + "px";
            f[0] = (n.offsetHeight + autoLayout_album.marginTop + parseInt(n.style.marginTop));
        }
        if (k.rightID) {
            for (var m = 0,
