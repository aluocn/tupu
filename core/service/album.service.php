...
class AlbumService
{
	...
	/**
	 * 获取会员是否已关注专辑
	 * by fishsword -- 2012-02-16
	 * @return bool
	 * */
	public function getIsFollow($aid,$uid)
	{
		global $_FANWE;
		$aid = (int)$aid;
		$uid = (int)$uid;
		if(!$aid || !$uid)
			return false;
		$is_follow = (int)FDB::resultFirst('SELECT COUNT(f_uid) FROM '.FDB::table('album_follow').' WHERE album_id = '.$aid.' AND f_uid = '.$uid);
	 ...
	 * 会员添加、取消 关注专辑
	 * by fishsword -- 2012-02-16
	 * @return int -1 自己的，1 关注成功，0取消关注成功
	 * */
	public function followAlbum($aid,$uid)
	{
		global $_FANWE;
		...
		if(empty($album) || $album['uid'] == $uid)
			return false;
		if(AlbumService::getIsFollow($aid,$uid))
		{
			FDB::query('DELETE FROM '.FDB::table('album_follow').' WHERE album_id = '.$aid.' AND f_uid = '.$uid);
			FDB::query('UPDATE '.FDB::table('album').' SET follow_count = follow_count - 1 WHERE id = '.$aid);
			return 0;
		}
		else
		{
			$album_data = array();
			$album_data['album_id'] = $aid;
			$album_data['f_uid'] = $uid;
			$album_data['create_time'] = TIME_UTC;
			
			FDB::insert('album_follow',$album_data);
	...
	public function updateAlbumByShare($aid,$share_id,$type = true)
	{
		...
		$paper_count  = (int)FDB::resultFirst('SELECT COUNT(DISTINCT photo_id) FROM '.FDB::table('share_photo').' 
			WHERE share_id = '.$share_id." and type =".'"paper"');
		
		if($paper_count > 0)//杂志社计算文章数
		{
			if($type)
				$update[] = 'paper_count = paper_count + '.$paper_count;
			else
				$update[] = 'paper_count = paper_count - '.$paper_count;
		}
	...
	/*格式化杂志社分类*/
	public function getAlbumCategory($id = 0,$icon = ''){
			$tmpstr = $icon;
			$sql="select id,parent_id,name from ".FDB::table('album_category')." where status=1 and  parent_id= $id";  
		    ...
		    if($result && mysql_affected_rows()){  
		        ...
		        while($rows=mysql_fetch_assoc($result)){  
		            ...
		            $rows['child']=$this->getAlbumCategory($rows['id']);  
		            foreach($rows['child'] as $k=>$v){
		            	$rows['child'][$k]['name'] = $tmpstr.$v['name'];
		            }
		            $arr[] = $rows;  
		        }  
		        return $arr;  
