...
class ShareService
{
	...
	public function submit($_POST,$is_check = true,$is_score = true,$is_album = FALSE)
	{
		...
		$share_data['is_video'] = intval($_POST['is_video']);
		...
		if($_POST['type'] == 'paper'){
			...
			$sdata['img'] = $_POST['pics'];
			$sdata['type'] = $_POST['type'];
			$sdata['sort'] = 10;
			$sdata['server_code']=$_POST['server_code'];
			$share_photos_data[] = $sdata;
		}else{
			$share_photos_data = array();
			//$_POST['pics']=array('a4b5geScB4pmTpwcPaNkGm99vsbzleui/WTG1hTQCmlJThp4WznmfW+I8junZ/hfMNbwgJxOZHzZ271SyAUX0knMrRhFZZz4R2jIGI+UoIOU89pD7P1vK1P94QVX7kGYB26q0jcOqmIomatYxf1bYccoKHOkoBWEZgtbxl10iUfudcWbuYMIbq3vwRJE5k15hBR5r9olUny2sR3v0gyUm9bTquiNKMZp+XLLmV8tqp1tkQ');
		...
		$data['text'] = $_POST['text'];//添加到share_photo的text字段数据
		$data['title'] = htmlspecialchars(trim($_POST['title']));//添加到share_photo的title字段数据
		$data['no_img'] = isset($_POST['no_img']) ? htmlspecialchars(trim($_POST['no_img'])) : 0;//文章分享没有图片
	...
	public function save($data,$is_score = true,$is_album = FALSE)
	{
		...
		if(intval($share_id)>0)
		{
			...
			foreach($share_photo as $k=>$photo)
			{
				...
				if(FS("Image")->getIsServer() && !empty($photo['server_code']))
				{
					...
					if(!empty($server))
					{
						...
						if(!empty($body))
						{
							...
							$weibo_img_url = FS("Image")->getImageUrl($o_img['url'],2);
						}else{
							return false;
						}
					}
				...
				{	
					if($photo['type'] == 'paper'){
						$o_img = copyImage($photo['img'],array(),'share',false,$share_id);
					}else{
				...
				if(!empty($o_img))
				{
					...
					if(!empty($photo['video'])){
						...
						$share_photo_data['video'] =  $photo['video'];
					}
					
					$share_photo_data['img_width'] =  $o_img['width'];
					$share_photo_data['is_animate'] =  $o_img['is_animate'];
					$share_photo_data['img_height'] =  $o_img['height'];
					$share_photo_data['server_code'] =  $photo['server_code'];
					$share_photo_data['text'] = $data['text'];
					$share_photo_data['title'] = $data['title'];
			...
			if($data['no_img']){//文章分享没有图片的时候
				
				$share_paper_data['uid'] = $_FANWE['uid'];
				$share_paper_data['share_id'] = $share_id;
				$share_paper_data['type'] =  'paper';
				$share_paper_data['sort'] =  10;
				$share_paper_data['text'] = $data['text'];
				$share_paper_data['title'] = $data['title'];
				FDB::insert('share_photo',$share_paper_data,true);
			}
			if(!empty($data['text'])){//如果是文章数量加1;
				$paper_count ++;
			}
			$shop_ids = array();
			$goods_count = 0;
			//保存分享的商品
			$is_video=$share_data['is_video'];
			
			if($photo_count > 0 && $is_video==1)
				$share_data_type = 'video';
			elseif($photo_count > 0 && $is_video==2)
				$share_data_type = 'goods';
			elseif($photo_count > 0 && $is_video==0)
				$share_data_type = 'photo';
			elseif($paper_count > 0 && $is_video==3)
				$share_data_type = 'paper';
			else
			...
			if($share_album_id > 0 && in_array($share_data_type,array('goods','photo','video','paper')))
			{
				...
				if($album)
				{
					...
					if($update_share['share_data'] != 'paper'){
			...
			if(!$is_album && $data['share']['type'] != 'fav')
				FDB::query('UPDATE '.FDB::table('user_count').' 
					SET shares = shares + 1,goods = goods + '.$goods_count.',photos = photos + '.$photo_count.',papers = papers + '.$paper_count.'
			...
			if(in_array($share_data_type,array('goods','photo','video','paper')))
			{
				//更新会员发布的有图分享编号
			...
			{
				if(!$is_rel_imgage && in_array($share_data_type,array('goods','photo','video')))
			...
			if($data['pub_out_check'])
			{
				...
				if($share_data['is_video']){
					...
					unset($weibo['img']);
					$weibo['content'] = $share_data['content'].''.$share_data['refer_url'];
				}
					...
					if($bind['sync'] && file_exists(FANWE_ROOT."login/".$class.".php"))
					{
						...
						*/
						...
						if($bind['sync'][$check_field] == 1)
						{
							...
							$re=FDB::insert('pub_schedule',$schedule,true);
	...
	public function deleteShare($share_id,$is_score = true)
	{
		...
		if(!empty($share))
		{
			...
			$image=FDB::fetchFirst("select * from ".FDB::table('share_photo')." where share_id=".$share_id);
			...
			if($image['server_code']){
				FS("Image")->deleteServerImg($image);			
			}else{
	...
	public function getShareImage($share_id,$data_type)
	{
		...
		switch($data_type)
		{
			...
			break;
			case 'photo':
				$sql = 'SELECT share_id,photo_id AS id,img,type AS ptype,\'m\' AS type,\'\' AS name,\'\' AS url,0 AS price,is_animate,video 
	 ...
	 * */
	public function getSameAlbumNextShares($uid,$share_id)
	{
		$arr = array('prev'=>0,'next'=>0);
		$share_ids = FS('User')->getSameAlbumIds($uid,$share_id);
	 ...
	 * @return array
	 * */
	public function getSameAlbumShares($share_id = 0,$num = 9)
	{
		if(!$share_id || $num == 0)
			return;
		...
		if(!$aid)
			return;
		$sql = 'select sp.img as img,als.share_id as share_id,sp.is_animate,s.content as content,sp.server_code as server_code from '.FDB::table('album_share').' as als 
				left join '.FDB::table('share_photo').' as sp on als.share_id = sp.share_id left join '.FDB::table('share').' as s on s.share_id = als.share_id 
				where als.album_id = '.$aid.' order by als.share_id desc limit '.$num;
		$sql_ids = 'select share_id from '.FDB::table('album_share').' where album_id ='.$aid.' order by share_id desc';
		
		$res_id = FDB::query($sql_ids);
		$ids = array();
		$indx = 1;
		while($data = FDB::fetch($res_id))
		{
			$ids[$indx] = $data['share_id'];
			$indx++;
		}
		$current_location = array_search($share_id,$ids); //当前分享所处位置
		$count = count($ids);
		$res = FDB::query($sql);
		$arr = array();
	...
	public function saveRelay($_POST)
	{
		...
		$data['share']['type'] = $type;//强制改成杂志社分享数据
		$data['share']['share_data'] = 'photo';//强制改成杂志社分享数据
		...
		$img_data = FDB::fetchFirst("select text,title,img,img_width,img_height,is_animate,video,server_code from ".FDB::table("share_photo")." where share_id = ".$share_id);
		...
		$share_photo_data['title'] = $img_data['title'];
		$share_photo_data['text'] = $img_data['text'];
		$share_photo_data['server_code']=$img_data['server_code'];
		if($share_photo_data['video']){
			$share_photo_data['type']='video';
		}else if($share_photo_data['text']){
			$share_photo_data['type'] =  'paper';
		}else{
