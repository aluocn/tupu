...
//			$imageServer=FS('Image')->getMinImgServerCode();
//			$server_code=$imageServer['code'];	
//		}
//		if($server_code){
//			$image = array();
//			$image['type'] = 'avatar';
//			$image['src'] = $_FANWE['site_url'].$img;
//			$image['server_code']=$server_code;
//			$image['sizes']=$types;//决定头像上传到图片服务器有几种规格的
//			$image['image']=$img;
//			$im=FS('Image')->addImage($image,$uid);
//			$server_code=$im['server_code'];
//			$sql="UPDATE ".FDB::table('user')." SET avatar_server_code = "."'$server_code'"." WHERE uid='$uid'";
//			FDB::query($sql);
//		}else{
		$uid = sprintf("%09d", $uid);
		$dir1 = substr($uid, 0, 3);
	 ...
	 * @return array
	 * */
	public function getSameAlbumIds($uid,$share_id)
	{
		$uid = (int)$uid;
		...
		$sql = 'select s.share_id as share_id from '.FDB::table('share').' as s 
				left join '.FDB::table('album_share').' as als on s.share_id = als.share_id 
				where s.uid = '.$uid.' and als.album_id = '.$aid.' order by s.share_id DESC';
	 ...
	 * 会员最新动态日志插入
	 *  by fishsword
	 * @param uid => 这条记录的拥有者 , object_id => 操作对象id ,  type => 日志类型 (1:关注人，2:喜欢，3:评论，4:提到，5:信件,6:采集,7:转采,8:关注杂志社) , create_time 创建时间
	 * @return bool
	 * */
	
	public function saveUserActivityLog($uid,$object_id,$type)
	{
		if(!$uid || !$object_id || !$type)
			return false;
		$data = array();
		$data['uid'] = $uid;
		...
		FDB::insert('user_activity_log',$data);
	}
