 ...
 * 获取数据库权限,是否能执行触发操作
 */
 function check_grant_core(){
 	return true;
 	global $_FANWE;
 	        $grants_array=FDB::fetchAll("SHOW GRANTS");
			$table_name=$_FANWE['config']['db'][1]['dbname'];
			foreach($grants_array as $k1=>$v1){
				...
				if(!empty($v)){
					...
					if( (strpos(strtolower($grant),'all')!==false)||(strpos(strtolower($grant),'super')!==false)||(strpos(strtolower($grant),'trriger')!==false) ){
						$re=FDB::fetchAll("SHOW TABLES LIKE '%_apns\_%' ");
						$m_num_1=count($re);
						$re=FDB::fetchAll("SHOW TABLES LIKE '".C("DB_PREFIX")."m\_%' ");
						$m_num_2=count($re);
						$all=$m_num_1+$m_num_2;
						if($all==8){
							return true;
...
function getImageType($url){ 
         ...
         if($fp = @fsockopen($url['host'],empty($url['port'])?80:$url['port'],$error)){ 
                 ...
                 while(!feof($fp)){ 
                         ...
                         if(trim($tmp) == ''){ 
                                 break; 
                         }else if(preg_match('/Content-Type: image\/(.*)/si',$tmp,$arr)){ 
                                 return trim($arr[1]); 
                         } 
                 } 
                 return null; 
...
function contentParse(&$content)
{
	...
	if(IS_UPYUN){
		...
		//添加2个匹配
		//$content = preg_replace("/\.\/public\/upload\/temp\//i",$image_url.'/public/upload/temp/', $content);
...
function clearSymbol($str)
{
	...
	if($symbols === NULL)
	{
		$symbols = file_get_url_g(PUBLIC_ROOT.'./table/symbol.table');
		$symbols = explode("\r\n",$symbols);
	}
...
function avatar_url($uid, $type = 'm', $is_src = 0)
{
	...
	if($avatars[$uid][$size] === NULL)
	{
		...
		$dir3 = substr($uid, 5, 2);
		$file_path = PUBLIC_ROOT.'./upload/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_'.$size.'.jpg';
		$file = $_FANWE['site_url'].'public/upload/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_'.$size.'.jpg';
		if(!file_exists($file_path))
			$file = $_FANWE['site_url'].'public/upload/avatar/noavatar_'.$size.'.jpg';

		$avatars[$uid][$size] = $file;
	}

	return $is_src ? $avatars[$uid][$size] : $avatars[$uid][$size];
}
 ...
 * by fzmatthew
 */
function getImgName($url,$width=0,$height=0,$gen = 0,$server_code='')
{
	static $imagec = NULL;
	global $_FANWE;
	if($width>0&&$height>0)
	{
		if($server_code){
			if($server_code == 'upyun'){
				$img_url=$img_url."!".$width."x".$height;
			}else{
				if(!isset($_FANWE['cache']['image_servers']))
					FanweService::instance()->cache->loadCache('image_servers');
					
				$current_img_server=$_FANWE['cache']['image_servers']['active'][$server_code];
				if(!$current_img_server){
					return ;
				}else{
					$img_url=FS('Image')->getImageUrl($url,1);
					$img_url_arr=explode('.',$img_url);
					$img_url=$img_url_arr[0]."_".$width."x".$height.".".$img_url_arr[1];
					if(!file_exists($img_url)){
						$server = FS("Image")->getServerByUrl($url);
						$img_url_arr=explode('.',$url);
						$url=$img_url_arr[1]."_".$width."x".$height.".".$img_url_arr[2];
						if($server['url_rewrite'] ==1){
							$img_url=$server['url'].str_replace('/'.$server['code'].'/','',$url);
						}else{
							$img_url = $server['url'].'getimg.php?img='.str_replace('/'.$server['code'].'/','',$url)."&gen=".$gen;
						}
					}
				}
			}
			...
			if(IS_UPYUN){
				//如果有upyun，则无需进行突破 的剪切
				if(strpos($url,'./upyun/') !== FALSE){
				$img_url_upyun=$url."!".$width."x".$height;
					return $img_url_upyun;
				}
			}
			$file_path=FANWE_ROOT.$url;
			if(!is_file($file_path))
				return;
			
			$org_img =$url;
			
			$img_file=explode('.',basename($url));
			$len=strlen($img_file[1]);
			if($img_file[1]=='jpg'){
				$img_file[1]='jpeg';
			}
			$type=get_image_info_g($file_path);
			$dir_name=dirname($url);
			$img_url=$dir_name."/".$img_file[0]."_".$width."x".$height.".".$type['type'];
			
			if(!is_file(FANWE_ROOT.$img_url))
			{
				if(class_exists('Imagick')){
					include_once fimport('class/imagick');
					$im=new lib_image_imagick();
					$im->open(FANWE_ROOT.$org_img);
					$scale_type="";
					if(!$gen){//等比缩放，非0裁剪
						$scale_type="scale";
					}
					if($type['type'] != 'gif'){
						if($width != 468 && $height!=468){//内容页面大图不做处理，gif格式的图片不做处理
							$im->resize_to($width,$height,$scale_type);	
						}	
					}
					
					
					$im->save_to(FANWE_ROOT.$img_url);
				}else{
					if($imagec === NULL)
					{
...
function copyImage($origin_path,$sizes = array(),$path = 'share', $del_temp = true,$id = 0){
	...
	makeDir(FANWE_ROOT.$dir);
	$type=get_image_type($origin_path);
	if(!$type){
		return false;
	}
	$file_name = md5(microtime(true)).random('6').'.'.$type;
	$file_path = FANWE_ROOT.$dir."/".$file_name;
	$bln = false;
	...
	else
	{
		...
		$data=@file_get_url_g($origin_path);
	...
	$is_animate=0;//是否为动态GIF图片
	if($bln)
	{
		
		$info  = get_image_info_g($file_path);
		...
		if($info['type'] == 'gif')
		{
			...
			*/
			...
			if($del_temp)
				@unlink($origin_path);
			$is_animate=IsAnimatedGif($file_path);
			if($is_animate){
			return array(
				'path' => $file_path,
				'url' => $dir."/".$file_name,
		...
		//对原图进行水印处理
		if($water_mark){
			if(class_exists('Imagick')){//运用imagick处理图片水印
				include_once fimport('class/imagick');
				$im=new lib_image_imagick();
				$im->open($water_image);
				$wwidth=$im->get_width();
				$wheight=$im->get_height();
				
				$image = new lib_image_imagick();  
				$image->open($file_path);
				$swidth=$image->get_width();
				$sheight=$image->get_height();
				
				if($swidth < $wwidth || $swidth < $wheight)
	            	return false;
				switch (intval($place))
		        {
		        	case 0: break;
		        	...
		        	case 1:
		        		$posY=0;
				        $posX=0;
				        $image->add_watermark($water_image, $posX, $posY);
				        break;
		        	//右上
		        	case 2:
		        		$posY=0;
		        	...
		        	//居中
		        	case 5:
				        $posY=$sheight/2-$wheight/2;
				        $posX=$swidth/2-$wwidth/2;
				        $image->add_watermark($water_image, $posX, $posY);
				        break;
		        }
				
				$image->save_to($file_path);
			}else{
				$image->water($file_path,$water_image,$alpha, $place);	
			}
		...
		if(IS_UPYUN){
			...
			$upload=$upyun->writeFile("/".$dir."/".$file_name, file_get_url_g($file_path), true) ;
			if($upload){
...
function content_parse_2($content){
	global $_FANWE;
	if(IS_UPYUN){
		...
		$replace = array (
			...
			$js_url.'public/js/',
			$image_url.'public/upload/',
			$_FANWE['site_url'].'public/',
			$_FANWE['site_url'].'public/data/tpl/css/',
			$_FANWE['site_url'].'tpl/'.TMPL.'/images/',
			$_FANWE['site_url'].'tpl/'.TMPL.'/js/',
			$_FANWE['site_url'].'tpl/\\1/',
		);
