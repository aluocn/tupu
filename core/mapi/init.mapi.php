...
class initMapi
{
	...
	function getAlbumCategoryArray()
	{
		global $_FANWE;
		FanweService::instance()->cache->loadCache('albums');
		$albumCategoryList = array();
		$indx = 0;
		foreach($_FANWE['cache']['albums']['category'] as $k => $v)
		{
			$albumCategoryList[$indx]['id'] = $v['id'];
			$albumCategoryList[$indx]['name'] = $v['name'];
			$indx ++;
 		}
    ...
    public function run()
	{
        ...
        */
		...
		$root['album_category'] = initMapi::getAlbumCategoryArray();
