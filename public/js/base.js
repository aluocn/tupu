...
(function($){
	...
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=all", {contentType:'ajax',draggable:false,showButton:false,title:"采集",width:1104,height:241});
	}
	//新增链接分享框
	$.Url_Share=function()
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=url", {contentType:'ajax',draggable:false,showButton:false,title:"网页分享",width:546,height:120});
	}
	//新增视频分享框
	$.Video_Share=function()
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=video", {contentType:'ajax',draggable:false,showButton:false,title:"视频分享",width:546,height:120});
	}
	//新增宝贝分享框
	$.Goods_Share=function()
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=goods", {contentType:'ajax',draggable:false,showButton:false,title:"宝贝分享",width:546,height:120});
	}
	//新建画板
	$.create_board=function()
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=create_board", {contentType:'ajax',draggable:false,showButton:false,title:"新建杂志社",width:800,height:350,type:'custom'});
	}
	//新增分享框
	...
	{
		$.weeboxs.close();
		$.weeboxs.open(SITE_PATH+"services/service.php?m=user&a=share", {contentType:'ajax',draggable:false,showButton:false,title:"发表新分享",width:546,height:120});
	}
	//编辑分享框
    ...
    $.Share_Video_Save=function(pageUrl,albumid,pic_url,video_url,title,pub_out_check){
		var query = new Object();
	...
	$.Share_Goods_Save=function(pageUrl,albumid,pic_url,title,pub_out_check){
		var query = new Object();
		query.pageUrl = pageUrl;
		query.imgArray = new Array(pic_url);
		query.album_id = albumid;
		query.title = title;
		query.content = title;
		query.is_video=2;	//0表示图片，1表示视频，2表示商品 
		query.pub_out_check=pub_out_check;
		var url = SITE_PATH + 'services/service.php?m=collectshare&a=save';
    ...
    $.Collect_Share_Save = function(obj,loc_url)//采集分享
	{
		...
		query.is_video = $("#is_video").val();
		//alert(query.pageUrl+","+query.imgArray.join("-")+""+query.imgArray+""+query.videoArray.join("-")+","+query.album_id+","+query.title+","+query.content);
		var pub_out_check = 0;
		
		if($("#checkbox-4",_parent).attr("checked") == 'checked')
		{
			pub_out_check = 1;
		}
		//alert($("#checkbox-4",_parent).attr("checked"));
		query.pub_out_check = pub_out_check;
	...
	/*=====================专辑关注BEGIN=====================*/
	$.Follow_Album = function(album_id,obj,flag)
	{
		if(!$.Check_Login())
			return false;
		var query = new Object();
		...
		$.ajax({
			...
			success:function(result){
				if(result.status == 1)
				{
					...
					{
						if(flag == 1){
							$(obj).removeClass('bts2');
							$(obj).addClass('bts1');
						}
						$(obj).html('<strong>取消关注</strong>');
					}
					else
					{
						if(flag == 1){
							$(obj).removeClass('bts1');
							$(obj).addClass('bts2');
						}
						$(obj).html('<strong>关注</strong>');
					}	
				}
			}
		})
	...
	//单独新建网格
	$.New_Create_Album_new=function(album_cate_id,title){
		var query=new Object();
		query.cid=album_cate_id;
		query.title=title;
		if(query.title == '' || query.title == LANG.new_album_name)
		{
			alert('名称不能为空');
		...
		$.ajax({
			...
			success: function(result){
				...
				{
					$.weeboxs.close();
					$.weeboxs.open(SITE_PATH+"services/service.php?m=public&a=success&type=create_album", {contentType:'ajax',draggable:false,showButton:false,title:"创建成功",width:310,height:200});
					url=result.url;
	...
	//新增的单独新建专辑
	$.Create_Album=function(album_cate_id,title){
		...
		if(query.title == '')
		{
			$(".CATE_NAME_TIP").css({"display":"block"});
			$(".CATE_NAME_TIP").html("请输入名称");
			$(".CATE_NAME_TIP").fadeOut(5000);
			return ;
		}
		if(query.cid == '')
		{
			$(".CATE_ID_TIP").css({"display":"block"});
			$(".CATE_ID_TIP").html("请选择分类");
			$(".CATE_ID_TIP").fadeOut(5000);
			return;
		}
		$.ajax({
			url: SITE_PATH+"services/service.php?m=share&a=savealbum",
...
//		}
	...
	$.Show_Bind_Change = function(){
		$.weeboxs.open(SITE_PATH+"services/service.php?m=u&a=bind_change", {contentType:'ajax',draggable:false,showButton:false,title:"友情提示",width:300,height:150});
	}
})(jQuery);
...
jQuery(function($){
	...
	/*登录后显示的下拉提示框*/
	$("#ntf_tip .shadow #ntf_strike").click(function(){
		$("#ntf_tip .shadow").css({"height":"1388px"});
		$("#ntf_strike").css({"display":"none"});
		$("#ntf_close").css({"display":"none"});
		$("#ntf_tip .shadow").children(".btt").css({"display":"block"});
		$("#ntf_tip .overlay a").css({"display":"block"});
	});
	$("#ntf_tip .overlay a").click(function(){
		$("#ntf_tip .shadow").css({"height":"34px"});
		$("#ntf_tip .btt").css({"display":"none"});
		$(this).css({"display":"none"});
		$("#ntf_strike").css({"display":"block"});
		$("#ntf_close").css({"display":"block"});
	});
	$("#ntf_close").click(function(){
		$("#ntf_tip").remove();
	});
	
	/*发表文章*/
	$("#paper_push").click(function(){
		var query = new Object();
		query.title = $.trim($("#title").val());
		query.albumid = $("#albumid").val();
		query.pub_out_check = $("input[name=share]").val();
		query.text = CKEDITOR.instances.TextArea1.getData();
		var msg = '';
		
		if(query.albumid == 0){
			msg +="请选择杂志社"+"\r\n";
		}
		if(query.title.length > 84){
			msg +="标题不能大于84个字符"+"\r\n";
		}
		if($.trim(query.text).length == 0){
			msg +="内容不能为空"+"\r\n";
		}else if($.trim(query.text).length < 100){
			msg +="内容不能小于100个字\r\n";
		}	
		if(msg.length > 0){
			alert(msg);
			...
			$.ajax({
				url: SITE_PATH+"services/service.php?m=share&a=savepaper",
				type: "POST",
				data:query,
				cache:false,
				dataType: "json",
	...
	$("#editpaper").click(function(){
		var query = new Object();
		query.title = $.trim($("#title").val());
		query.albumid = parseInt($("#albumid").val());
		query.share_id = parseInt($("#share_id").val());
		query.photo_id = parseInt($("#photo_id").val());
		query.old_albumid = parseInt($("#old_album_id").val());
		query.server_code = $("#server_code").val();
		query.text = CKEDITOR.instances.TextArea1.getData();
		var msg = '';
		
		
		if(isNaN(query.albumid) || query.albumid == 0){
			msg +="请选择杂志社"+"\r\n";
		}
		if(query.title.length > 84){
			msg +="标题不能大于84个字符"+"\r\n";
		...
		if(msg.length > 0){
			...
			$.ajax({
				url:SITE_PATH+"services/service.php?m=share&a=updatepaper",
				type:"POST",
				data:query,
				cache:false,
				dataType:'json',
	...
	$(function(){
		$(window).scroll(function(){
            if($(window).scrollTop() > 39){
               ...
               $("#register-tip").css({"position":"fixed"});
               $("#register-tip").css({"width":"100%"});
               $("#register-tip").css({"z-index":"9999"});
               $("#register-tip").css({"bottom":"0"});
               $("#register-tip").css({"left":"0"});
