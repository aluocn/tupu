...
class AlbumCategoryAction extends CommonAction
{
	public function index(){
		...
		if(!empty($model)){
			...
			$sql = 'SELECT ac.*,COUNT(ac1.id) AS child 
				FROM '.C("DB_PREFIX").'album_category as ac 
				LEFT JOIN '.C("DB_PREFIX").'album_category as ac1 ON ac1.parent_id = ac.id 
				WHERE ac.parent_id = '.$parent_id.' GROUP BY ac.id';
	...
	public function edit()
	{
		...
		$vo = D("AlbumCategory")->getById($id);
		$this->assign ( 'vo', $vo );
		$childs = D("AlbumCategory")->getChildIds($id,'id','parent_id');
		$childs[] = $id;
		$cate_list = D("AlbumCategory")->where('status = 1 AND id not in ('.implode(',', $childs).')')->field('id,parent_id,name')->order('sort ASC,id ASC')->select();
		$cate_list = D("AlbumCategory")->toFormatTree($cate_list,array('name'),'id','parent_id');
		$this->assign("cate_list",$cate_list);
		$this->display();
	}
	
	public function add(){
		$cate_list = D("AlbumCategory")->where('status = 1')->field('id,parent_id,name')->order('sort ASC,id ASC')->select();
		$cate_list = D("AlbumCategory")->toFormatTree($cate_list,'name','id','parent_id');
		$this->assign("cate_list",$cate_list);
		$this->display(); 
	}
	...
	function getChildLink($id,$cate)
	{
		if($cate['child'] > 0){
			return '<a href="'.U('AlbumCategory/index',array('parent_id'=>$id)).'">'.L('SHOW_CHILD').'</a>';
		}else{
			return;
		}
